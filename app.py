...
TEN_CX = {0: "Tiêu cực", 1: "Trung lập", 2: "Tích cực"}   # mã gốc UIT-VSFC
...
trung_lap = int((df_loc["sentiment"] == 1).sum())
tich_cuc = int((df_loc["sentiment"] == 2).sum())
...
with col_p:
    ...
    tu_tich_cuc = top_tu(df_loc[df_loc["sentiment"] == 2])
    if tu_tich_cuc:
